...
| seed | int | Seed for the random number generator that initializes network weights. |
## train
...
``` python
def visualize_optimized_inputs(self, data, layer_name, output_file, nodes = None)
```
Visualize what every node in the network has learned. 
 ...
 Warning: This kind of visualization has been applied before to image classification networks and while the resulting images are usually somewhat recognizable they are still very hard to interpret (e.g. http://ankivil.com/visualizing-deep-neural-networks-classes-and-features/). For a PWM to be useful it has to be very precise, but this is unfortunately not the case for many data sets. Therefore this function should not be considered for any biological interpretations. Please use the visualize\_kernel() method for more reliable visualizations. Nevertheless, visualization of all layers of a network can be interesting if you are interested in neural networks per se. 
...
| parameter | type | description |
|:-|:-|:-|
| data | placeholder.Data | The Data object used to train the model. |
